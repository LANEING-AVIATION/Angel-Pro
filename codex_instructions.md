...
* **[Pending Human Review: Verify Empty Cupertino Workspace Effect] Task 0.1:** Replace the prototype entry widget with a minimal, directly launched `CupertinoApp` workspace shell containing only a full-window empty canvas; confirm that no splash, file-manager, Material widget, toolbar, menu, or legacy route is reachable.
  * **Implementation record:** The active Dart entry point now launches only the black `WorkspaceCanvas`; prototype WebView/startup-document code, its temporary dependency, and its temporary asset registrations were removed until their dedicated roadmap tasks. Windows debug compilation is stabilized through a project-local Flutter backend wrapper that restores the required system/Git PATH and tolerates localized subprocess output, and the existing shell-association code now includes its required Windows Shell header.
